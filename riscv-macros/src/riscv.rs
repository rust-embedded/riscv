//! Auxiliary structures and functions for proc macros that depend on the `riscv` crate.
...
impl PacEnumItem {
    ...
    /// Returns a vector of token streams representing the trait implementations for the enum.
    ///
    /// If the `rt` feature is enabled and the trait is an interrupt trait, the implementation also includes
    /// the external declaration of interrupt handler functions and the interrupt array.
    ///
    /// If the `rt-v-trap` feature is enabled and the trait is a core interrupt trait, it also includes
    /// the vector table definition.
    pub fn impl_trait(&self, attr: &PacTrait) -> Vec<TokenStream2> {
        let mut res = vec![];
        ...
        #[cfg(feature = "rt")]
        self.push_rt_code(attr, &mut res);
...
#[cfg(feature = "rt")]
mod rt {
    //! Auxiliary structures and functions for generating runtime code depending on the `riscv` and `riscv-rt` crates.
    ...
    impl PacEnumItem {
        /// Appends to `code` the necessary runtime code for exception or interrupt handling.
        pub(super) fn push_rt_code(&self, pac: &PacTrait, code: &mut Vec<TokenStream2>) {
            if let Some(trap_config) = pac.trap_config() {
                match pac {
                    #[cfg(feature = "rt-v-trap")]
                    PacTrait::Interrupt(InterruptType::Core) => {
                        code.push(self.vector_table());
                    }
                    _ => {
                        let max_discriminant = self.max_number;
        ...
        /// Otherwise, it is `Some(<InterruptName>)`.
        ...
        /// Generates the vector table for core interrupts when the `rt-v-trap` feature is enabled.
        ///
        /// If an interrupt number is not present in the enum, the corresponding entry jumps to the `_start_DefaultHandler_trap` label.
        /// Otherwise, it jumps to the `_start_<InterruptName>_trap` label.
        #[cfg(feature = "rt-v-trap")]
